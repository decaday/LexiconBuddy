...
impl Dict {
    fn build(dict_type: DictTypes) -> Result<Dict, DictError> {
     ...
     */
    fn search_index(mut self, entry: &str) -> Result<u64, DictError> {
        let mut low = 0;
        ...
        while low <= high {
            ...
            self.index_file.read_exact(&line);
            let mut line = String::new();
...
fn run() -> Result<u64, DictError> {
    let entry = "apple";
    let ecdict = Dict::build(DictTypes::Ecdict)?;
...
fn main() {
    match run() {
        ...
        Err(error) => match error {
            DictError::IOError(e) => println!("IO error! {e}"),
            DictError::ParseError(e) => println!("ParseError {e}"),
            DictError::ParseIntError(e) => println!("ParseError {e}"),
            DictError::WordNotFound => println!("Word not fond"),
        }
    };
}
